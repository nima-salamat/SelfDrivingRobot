...
class ApriltagDetector:
    def __init__(self):
        # Define the dictionary for ArUco marker
        self.aruco_dict = cv2.aruco.Dictionary_get(cv2.aruco.DICT_6X6_250)
        self.parameters = cv2.aruco.DetectorParameters_create()

    def detect(self, frame) -> list:
        """
        Detects ArUco markers in the given frame. (Simulated AprilTags with ArUco)
        ...
        if len(corners) > 0:
            for i in range(len(corners)):
                ...
                if AT_LEFT_ROI <= min_x / frame.shape[1] <= AT_RIGHT_ROI and AT_TOP_ROI <= min_y / frame.shape[0] <= AT_BOTTOM_ROI:
                    ...
                    cv2.putText(frame, f"ID: {ids[i][0]}", (int((min_x + max_x) / 2), int((min_y + max_y) / 2)), 
