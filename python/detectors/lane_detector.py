...
class VisionProcessor:
    """Refactored VisionProcessor.

    - Supports modes: 'race' (only left/right lanes) and 'city' (lanes + crosswalk + apriltag + traffic light)
    - debug flag controls drawing on returned frame (if False, returned frame will be None)
    - Public API: detector = VisionProcessor(config, mode='race', debug=True)
                  steering, frame, info = detector.detect(frame)
    """
    ...
    def detect(self, frame: np.ndarray) -> Tuple[float, np.ndarray, Dict[str, object]]:
        """Main detect method. Chooses behavior based on mode and returns:
        (steering_angle, debug_frame_or_None, info_dict)
        """
        ...
        # city mode: full processing
            ...
            steering_angle, centers, cw_count, at_info, tl_color, longest = self._detect_city(frame, rois)
            info.update({
                "center_right_x": centers[0],
                "center_left_x": centers[1],
                "cw_lines": cw_count,
    ...
    def _detect_city(self, frame: np.ndarray, rois: Dict[str, Tuple[int, int, int, int]]):
        # lanes
        top_rl, bottom_rl, left_rl, right_rl = rois['rl']
        roi_frame_rl = frame[top_rl:bottom_rl, left_rl:right_rl]
        center_right_x, right_lines_info, longest_r = self._process_lane_roi(roi_frame_rl, left_rl, right_rl, is_right=True)
        ...
        at_tags_info = self._process_apriltag_roi(roi_frame_at, left_at, top_at)
            ...
            for line in lines:
                for x1, y1, x2, y2 in line:
                    ...
                    if abs(slope) > 0.3:
                        ...
                        x_bottom = x1 + (height - y1) / (slope if abs(slope) > 1e-5 else 1)
                        x_bottom = max(0, min(right_offset - left_offset - 1, x_bottom))
                        x_bottom_full = x_bottom + left_offset
                        if is_right and x_bottom_full >= (left_offset + right_offset) // 2:
                            lines_info.append((x1, y1, x2, y2, x_bottom_full, length))
                        elif not is_right and x_bottom_full <= (left_offset + right_offset) // 2:
                            lines_info.append((x1, y1, x2, y2, x_bottom_full, length))
    ...
    def _process_crosswalk_roi(self, roi_frame: np.ndarray) -> List[tuple]:
        gray = cv2.cvtColor(roi_frame, cv2.COLOR_RGB2GRAY)
        edge = cv2.Canny(gray, self.config['CANNY_LOW'], self.config['CANNY_HIGH'])
        lines = cv2.HoughLinesP(edge, 1, np.pi / 180, self.config['HOUGH_THRESHOLD'], minLineLength=20, maxLineGap=self.config['MAX_LINE_GAP'])
        cw_lines = []
        if lines is not None:
            for line in lines:
                for x1, y1, x2, y2 in line:
    ...
    def _process_apriltag_roi(self, roi_frame: np.ndarray, left_offset: int, top_offset: int) -> List[tuple]:
        gray = cv2.cvtColor(roi_frame, cv2.COLOR_RGB2GRAY)
        corners, ids, rejected = aruco.detectMarkers(gray, self.aruco_dict, parameters=self.aruco_params)
        tags = []
        if ids is not None and len(ids) > 0:
            aruco.drawDetectedMarkers(roi_frame, corners, ids)
    ...
    # ---------------------- Drawing helpers ----------------------
    def _draw_lane_visuals(self, frame: np.ndarray, rois: Dict[str, Tuple[int, int, int, int]], centers: tuple, lane_type: str = "both"):
        if frame is None:
            return
        left_rl = rois['rl'][2]
        right_rl = rois['rl'][3]
        ...
        # Right ROI (yellow)
        cv2.rectangle(frame, (left_rl, top_rl), (right_rl, bottom_rl), (0, 255, 255), 2)
        cv2.putText(frame, "RL", (left_rl + 5, top_rl + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        # Left ROI (cyan)
        cv2.rectangle(frame, (left_ll, top_ll), (right_ll, bottom_ll), (255, 255, 0), 2)
        cv2.putText(frame, "LL", (left_ll + 5, top_ll + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        # Draw lane center lines
        cv2.line(frame, (int(center_right_x), top_rl), (int(center_right_x), bottom_rl), (255, 0, 0), 2)
        cv2.line(frame, (int(center_left_x), top_ll), (int(center_left_x), bottom_ll), (0, 0, 255), 2)

    def _draw_city_visuals(self, frame: np.ndarray, rois: Dict[str, Tuple[int, int, int, int]], centers: tuple, cw_info_count: int, at_info: List[tuple], tl_color: str):
        if frame is None:
            return
        # draw lane + cw + at + tl overlays
        self._draw_lane_visuals(frame, rois, centers)
        ...
        # Crosswalk ROI (green)
        cv2.rectangle(frame, (left_cw, top_cw), (right_cw, bottom_cw), (0, 255, 0), 2)
        cv2.putText(frame, "CW", (left_cw + 5, top_cw + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        # AprilTag ROI (green)
        cv2.rectangle(frame, (left_at, top_at), (right_at, bottom_at), (0, 255, 0), 2)
        cv2.putText(frame, "AT", (left_at + 5, top_at + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        # Traffic light ROI (magenta)
        cv2.rectangle(frame, (left_tl, top_tl), (right_tl, bottom_tl), (255, 0, 255), 2)
        cv2.putText(frame, "TL", (left_tl + 5, top_tl + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)

        # Draw AprilTag boxes and ids
        for tag_id, corners, area in at_info:
            ...
            cv2.putText(frame, f"ID: {tag_id}", (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Draw traffic light color
        cv2.putText(frame, f"Color: {tl_color}", (left_tl + 5, top_tl + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

        # stats
        cv2.putText(frame, f"CW Lines: {cw_info_count}", (left_cw + 5, top_cw + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        cv2.putText(frame, f"AT Tags: {len(at_info)}", (left_at + 5, top_at + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
